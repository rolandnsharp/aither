...
signal.sleep = function(ms) {
  return new Promise(resolve => setTimeout(resolve, ms));
};

// Bar-based composition helpers
let compositionStartTime = null;

// Get current bar number
signal.getCurrentBar = function(bpm) {
  if (!compositionStartTime) {
    compositionStartTime = Date.now() / 1000; // Convert to seconds
  }
  const currentTime = Date.now() / 1000;
  const elapsed = currentTime - compositionStartTime;
  const secondsPerBar = (60 / bpm) * 4; // 4 beats per bar
  return Math.floor(elapsed / secondsPerBar);
};

// Wait for N bars (promise-based)
signal.bars = function(numBars, bpm = 120) {
  const secondsPerBar = (60 / bpm) * 4;
  const ms = numBars * secondsPerBar * 1000;
  return new Promise(resolve => setTimeout(resolve, ms));
};

// Trigger callback on specific bar
signal.onBar = function(barNumber, bpm = 120, callback) {
  if (!compositionStartTime) {
    compositionStartTime = Date.now() / 1000;
  }

  const secondsPerBar = (60 / bpm) * 4;
  const targetTime = compositionStartTime + (barNumber * secondsPerBar);
  const currentTime = Date.now() / 1000;
...
// Loop every N bars
signal.loop = function(numBars, bpm = 120, callback) {
  if (!compositionStartTime) {
    compositionStartTime = Date.now() / 1000;
  }

  const secondsPerBar = (60 / bpm) * 4;
  const intervalMs = numBars * secondsPerBar * 1000;

  const interval = setInterval(() => {
    callback(signal.getCurrentBar(bpm));
  }, intervalMs);

  // Call immediately for first iteration
  callback(signal.getCurrentBar(bpm));

  return interval; // Return so it can be cleared
};

// Reset composition timer (for new compositions)
signal.resetTimer = function() {
  compositionStartTime = Date.now() / 1000;
};
